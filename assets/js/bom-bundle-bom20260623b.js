...
(function (global) {
  ...
  function loadScript(url, optional) {
    return new Promise(function (resolve, reject) {
      ...
      s.onerror = function () {
        ...
        if (optional) {
          if (global.console && console.warn) console.warn('[BOM clean bundle]', msg);
          resolve();
        } else {
  ...
  function hardenConfig() {
    ...
    cfg.THREE_DPLAY = {
      ...
      WIDGET_HINT: '3DPlay desativado: aguardando Geometry Resolver real.'
    };
  ...
  function ensureLoaded() {
    ...
    setStatus('Carregando bundle operacional limpo...', 'info');
    var root = baseUrl();
    loading = modules.reduce(function (p, path) {
      ...
      .then(function () {
        ...
        if (!realController || !realController.boot) {
          throw new Error('Controller oficial nao foi carregado pelo bundle limpo.');
        }
        setStatus('Bundle operacional limpo carregado. 3DPlay desativado.', 'success');
        return realController;
      })
      .catch(function (error) {
        setStatus(error && error.message ? error.message : 'Erro no bundle operacional limpo.', 'error');
        throw error;
      });
    return loading;
  }
  ...
  var proxy = {
    ...
    getState: function () {
      ...
      return {
        controller: 'bom-clean-bundle-loader',
        activeBuild: BUILD,
        cleanBundleLoaded: loaded,
