...
(function (global) {
  ...
  function hardenConfig() {
    ...
    cfg.EXPAND_PATH_PARSER = {
      ENABLED: true,
      VERSION: 'bom20260623h',
      CONTRACT: 'member[] + Path[]',
      PATH_RULE: 'last=referenceId, previous=instanceId, previous=parentReferenceId, level=(length-1)/2'
    };
    cfg.THREE_DPLAY = {
      ENABLED: false,
      EMBED_PLAYER: false,
    ...
    cfg.ATTRIBUTE_ENRICHMENT = {
      ENABLED: true,
      SOURCE: 'Path parser referencedObject + EngItem reread when needed',
  ...
  function ensureLoaded() {
    ...
    setStatus('Carregando bundle operacional Path parser...', 'info');
    var root = baseUrl();
    loading = modules.reduce(function (p, path) {
      ...
      .then(function () {
        hardenConfig();
        if (global.BomExpandPathParser && global.BomExpandPathParser.install) {
          global.BomExpandPathParser.install();
        }
      ...
      .then(function () {
        ...
        if (!realController || !realController.boot) {
          throw new Error('Controller oficial nao foi carregado pelo bundle Path parser.');
        }
        refreshAutoContextStatus();
        setStatus('Bundle Path parser carregado. Abra a estrutura no Product Explorer e clique Atualizar estrutura.', 'success');
        return realController;
      })
      .catch(function (error) {
        setStatus(error && error.message ? error.message : 'Erro no bundle Path parser.', 'error');
        throw error;
      });
    return loading;
  }
