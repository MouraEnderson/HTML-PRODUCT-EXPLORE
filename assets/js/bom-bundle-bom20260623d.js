/* BOM Analytics bom20260623d-ebom-first
 * Operational bundle loader: controller-only path, WAFData session, no 3DPlay/iframe/mock success.
 * Fixes PR3 gating: Geometry Resolver appears only after a real E-BOM row is selected.
 * Adds explicit manual-root input when Product Explorer auto-context is unavailable.
 */
(function (global) {
  'use strict';

  var BUILD = 'bom20260623d';
  ...
  var modules = [
    ...
    'assets/js/bom-inline-root-loader-bom20260623d.js',
    'assets/js/geometry/bom-geometry-resolver-bom20260623c.js',
    'assets/js/geometry/bom-geometry-controller-patch-bom20260623d.js'
  ];
  ...
  function hardenConfig() {
    ...
    cfg.ROOT_INPUT = {
      ENABLED_WHEN_AUTO_CONTEXT_UNAVAILABLE: true,
      ACCEPTS: ['dseng-id', 'prd-R-physical-id', 'exact-title'],
      NO_SILENT_CJ_FALLBACK: true
    };
  }
  ...
  function ensureLoaded() {
    ...
    setStatus('Carregando bundle operacional E-BOM-first...', 'info');
    var root = baseUrl();
    loading = modules.reduce(function (p, path) {
      ...
      .then(function () {
        ...
        if (!realController || !realController.boot) {
          throw new Error('Controller oficial nao foi carregado pelo bundle E-BOM-first.');
        }
        setStatus('Bundle E-BOM-first carregado. 3D real exige linha E-BOM selecionada.', 'success');
        return realController;
      })
      .catch(function (error) {
        setStatus(error && error.message ? error.message : 'Erro no bundle E-BOM-first.', 'error');
        throw error;
      });
    return loading;
  }
  ...
  var proxy = {
    ...
    getState: function () {
      ...
      return {
        controller: 'bom-ebom-first-bundle-loader',
        activeBuild: BUILD,
        geometryBundleLoaded: loaded,
        bootRequested: bootRequested,
        no3DPlayOperational: true,
        geometryResolverRequiresSelectedRow: true,
        manualRootInputVisibleWhenAutoContextUnavailable: true
      };
    },
